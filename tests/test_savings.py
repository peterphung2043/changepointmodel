import pytest
import numpy as np
from energymodel.savings import AshraeAdjustedSavingsCalculator, \
...
def test_adjusted_savings_calls_adjusted_correctly(mocker): 
    ...
    mocker.patch('energymodel.savings._get_adjusted', return_value=np.array([42.,]))
    mock = mocker.patch('energymodel.calc.savings.adjusted', return_value=(1,1,1,1,))
...
def test_normalized_savings_calls_weather_normalzied_correctly(mocker): 
    ...
    mocker.patch('energymodel.savings._get_adjusted', return_value=np.array([42.,]))
    mock = mocker.patch('energymodel.calc.savings.weather_normalized', return_value=(1,1,1,1,))
