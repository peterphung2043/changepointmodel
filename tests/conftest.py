...
def baseline_request(raw_energy_model_data):
    ...
    return {'model_config': dict(models=models, model_filter=model_filter), 'usage': usage}
...
def option_c_request(raw_energy_model_data):
    ...
    pre = dict(model_config=dict(models=models, norms=norms, model_filter=model_filter), usage=raw_energy_model_data['pre_data'])
    post = dict(model_config=dict(models=models, norms=norms, model_filter=model_filter), usage=raw_energy_model_data['post_data'])
...
@dataclass
class DummResult:
    scores: Optional[List[DummyScore]]=None #making it optional so that testing is eaiser for filter extras
    coeffs: Optional[DummyCoefficients]=None#making it optional so that test dont break
    pred_y: Optional[List[float]]=None #making it optional so that test dont break
    input_data: Optional[DummyInputData]=None #making it optional so that test dont break 
...
def dummy_result_for_filter_extra_good_for_shape_test():
    ...
    coeffs_5p = DummyCoefficients(yint=1, slopes=[-0.5,1], changepoints=[5,6]) #making this bad intentionally to test slopes comparison
...
def dummy_result_for_filter_extra_good_for_dpop_test():
    data = DummyInputData(X=np.array([i for i in range(1,13)]), y=np.array([i for i in range(1,13)]))
    coeffs_2p = DummyCoefficients(yint=1, slopes=[1], changepoints=None)
    coeffs_3pc = DummyCoefficients(yint=1, slopes=[1], changepoints=[5])
    coeffs_3ph = DummyCoefficients(yint=1, slopes=[-1], changepoints=[5])
    coeffs_4p = DummyCoefficients(yint=1, slopes=[-1.5,1], changepoints=[5])
...
def dummy_t_test_good():
    ...
    result_3pc = DummResult(coeffs=coeffs_3pc,name='3PC', input_data=data_3pc, pred_y=y_3pc)
    result_3ph = DummResult(coeffs=coeffs_3ph, name='3PH', input_data=data_3ph, pred_y=y_3ph)
...
def dummy_t_test_bad():
    ...
    result_2p = DummResult(coeffs=coeffs_2p, name='2P', input_data=data_2p, pred_y=y_2p)
    result_3pc = DummResult(coeffs=coeffs_3pc,name='3PC', input_data=data_3pc, pred_y=pred_y_3pc)
    result_3ph = DummResult(coeffs=coeffs_3ph, name='3PH', input_data=data_3ph, pred_y=pred_y_3ph)
    result_4p = DummResult(coeffs=coeffs_4p, name='4P', input_data=data_4p, pred_y=pred_y_4p)
    result_5p = DummResult(coeffs=coeffs_5p, name='5P', input_data=data_5p, pred_y=pred_y_5p)
