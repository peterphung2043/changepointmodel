import numpy as np
from ashrae._lib import bounds
...
def test_fourp_bounds():
    ...
    assert res == ((0, -np.inf, -np.inf, 3.2040816326530615), (np.inf, np.inf, np.inf, 7.795918367346939))


def test_fivep_bounds(): 
    check_X = np.linspace(1., 10.)
    res = bounds.fivep(check_X) 
    assert res == ((0, -np.inf, 0, 3.2040816326530615, 6.6938775510204085), (np.inf, 0, np.inf, 4.3061224489795915, 7.795918367346939))
