...
def test_EnergyChangepointModelResult_parse(changepointmodel_response_fixture):
    ...
    measurements = ['base_load', 'heating_load', 'cooling_load', 'cooling_sensitivity',
                'heating_sensitivity', 'heating_changepoint', 'cooling_changepoint',
                'normalized_annual_consumption', 'r2', 'r2_threshold',
                'r2_ok', 'cvrmse', 'cvrmse_threshold', 'cvrmse_ok', 'model_type']
    ...
    none_keys = ['heating_sensitivity', 'heating_changepoint', 'cooling_changepoint',
    ...
    not_none_keys = ['r2', 'r2_threshold', 'r2_ok', 'cvrmse', 'cvrmse_threshold',
                        'cvrmse_ok', 'base_load', 'cooling_load']
...
def test_SavingsResult_parse(savings_fixture):
    ...
    saving_result_measure_types = {'average_savings', 'percent_savings', 'percent_savings_uncertainty', 'total_savings'}
...
def test_SavingsResponse_parsing(savings_fixture):
    ...
    model_output, savings_results_output, savings_related_usage_output, norms_output = result.parse_results_for_csv()
...
def test_EnergyParameterModelCoefficients_parsing_3PC():
    ...
    parsed = load_result.parse(model_type)
    assert len(parsed) == 2 #two measurementtypes for 3PC; cooling sensitivity and cooling changepoint
...
def test_EnergyParameterModelCoefficients_parsing_3PH():
    ...
    parsed = load_result.parse(model_type)
    assert len(parsed) == 2 #two measurementtypes for 3PH; heating sensitivity and heating changepoint
...
def test_EnergyParameterModelCoefficients_parsing_4P():
    ...
    parsed = load_result.parse(model_type)
    assert len(parsed) == 4 #four measurementtypes for 4P; sensitivity and changepoint both cooling and heating; but same changepoint
...
def test_EnergyParameterModelCoefficients_parsing_5P():
    ...
    parsed = load_result.parse(model_type)
    assert len(parsed) == 4 #four measurementtypes for 5P; sensitivity and changepoint both cooling and heating; not the same changepoint
