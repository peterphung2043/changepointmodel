...
def test_postive_sum_only_sums_positives(): 
    assert loads._positive_sum(np.array([-1., 0., 1.])) == 1
...
def test_coolingload_calculates_correct_predicted_load(): 
    ...
    assert [0.,0.,0.,0.,0.,4.,5.,6.,7.,8.,] == res.tolist()
...
def test_heatingload_calculates_correct_predicted_load(): 
    ...
    assert [8.,7.,6.,5.,0.,0.,0.,0.,0.,0.] == res.tolist()
...
def test_coolingload_returns_correct_result(mocker): 
    ...
    np.testing.assert_array_equal(calls[0][0][0], [0.,0.,0.,0.,0.,4.,5.,6.,7.,8.,])  # XXX prob should make a helper function out of this
...
def test_heatingload_correct_calling_behavior(mocker): 
    ...
    calls = sumspy.call_args_list
    np.testing.assert_array_equal(calls[0][0][0], [8.,7.,6.,5.,0.,0.,0.,0.,0.,0.])